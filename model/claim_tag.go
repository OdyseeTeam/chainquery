...
func (r *claimTagR) GetClaim() *Claim {
	if r == nil {
		return nil
	}
	return r.Claim
...
func (claimTagL) LoadClaim(e boil.Executor, singular bool, maybeClaimTag interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeClaimTag.(*ClaimTag)
		if !ok {
			object = new(ClaimTag)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeClaimTag)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeClaimTag))
			}
		}
	} else {
		s, ok := maybeClaimTag.(*[]*ClaimTag)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeClaimTag)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeClaimTag))
			}
		}
	}
...
func (claimTagL) LoadTag(e boil.Executor, singular bool, maybeClaimTag interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`tag`),
		qm.WhereIn(`tag.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *ClaimTag) Upsert(exec boil.Executor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(claimTagAllColumns, strmangle.SetIntersect(insert, update))
...
// Exists checks if the ClaimTag row exists.
func (o *ClaimTag) Exists(exec boil.Executor) (bool, error) {
	return ClaimTagExists(exec, o.ID)
}

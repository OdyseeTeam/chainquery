...
func (r *transactionAddressR) GetTransaction() *Transaction {
	if r == nil {
		return nil
	}
...
func (r *transactionAddressR) GetAddress() *Address {
	if r == nil {
		return nil
	}
	return r.Address
...
func (transactionAddressL) LoadTransaction(e boil.Executor, singular bool, maybeTransactionAddress interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeTransactionAddress.(*TransactionAddress)
		if !ok {
			object = new(TransactionAddress)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeTransactionAddress)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeTransactionAddress))
			}
		}
	} else {
		s, ok := maybeTransactionAddress.(*[]*TransactionAddress)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeTransactionAddress)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeTransactionAddress))
			}
		}
	}
...
func (transactionAddressL) LoadAddress(e boil.Executor, singular bool, maybeTransactionAddress interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`address`),
		qm.WhereIn(`address.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *TransactionAddress) Upsert(exec boil.Executor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(transactionAddressAllColumns, strmangle.SetIntersect(insert, update))
...
// Exists checks if the TransactionAddress row exists.
func (o *TransactionAddress) Exists(exec boil.Executor) (bool, error) {
	return TransactionAddressExists(exec, o.TransactionID, o.AddressID)
}

...
func (r *claimInListR) GetListClaim() *Claim {
	if r == nil {
		return nil
	}
	return r.ListClaim
...
func (claimInListL) LoadListClaim(e boil.Executor, singular bool, maybeClaimInList interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeClaimInList.(*ClaimInList)
		if !ok {
			object = new(ClaimInList)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeClaimInList)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeClaimInList))
			}
		}
	} else {
		s, ok := maybeClaimInList.(*[]*ClaimInList)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeClaimInList)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeClaimInList))
			}
		}
	}
	...
	if singular {
		...
		args[object.ListClaimID] = struct{}{}
	...
	query := NewQuery(
		qm.From(`claim`),
		qm.WhereIn(`claim.claim_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *ClaimInList) Upsert(exec boil.Executor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(claimInListAllColumns, strmangle.SetIntersect(insert, update))
...
// Exists checks if the ClaimInList row exists.
func (o *ClaimInList) Exists(exec boil.Executor) (bool, error) {
	return ClaimInListExists(exec, o.ID)
}

...
func (r *abnormalClaimR) GetOutput() *Output {
	if r == nil {
		return nil
	}
	return r.Output
...
func (abnormalClaimL) LoadOutput(e boil.Executor, singular bool, maybeAbnormalClaim interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeAbnormalClaim.(*AbnormalClaim)
		if !ok {
			object = new(AbnormalClaim)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAbnormalClaim)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAbnormalClaim))
			}
		}
	} else {
		s, ok := maybeAbnormalClaim.(*[]*AbnormalClaim)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAbnormalClaim)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAbnormalClaim))
			}
		}
	}
	...
	query := NewQuery(
		qm.From(`output`),
		qm.WhereIn(`output.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *AbnormalClaim) Upsert(exec boil.Executor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(abnormalClaimAllColumns, strmangle.SetIntersect(insert, update))
...
// Exists checks if the AbnormalClaim row exists.
func (o *AbnormalClaim) Exists(exec boil.Executor) (bool, error) {
	return AbnormalClaimExists(exec, o.ID)
}

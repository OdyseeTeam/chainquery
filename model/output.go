...
var OutputTableColumns = struct {
	...
	Vout:               "output.vout",
	Type:               "output.type",
	ScriptPubKeyAsm:    "output.script_pub_key_asm",
	ScriptPubKeyHex:    "output.script_pub_key_hex",
	RequiredSignatures: "output.required_signatures",
	AddressList:        "output.address_list",
	IsSpent:            "output.is_spent",
	SpentByInputID:     "output.spent_by_input_id",
	CreatedAt:          "output.created_at",
	ModifiedAt:         "output.modified_at",
...
type outputR struct {
	...
	AbnormalClaims AbnormalClaimSlice `boil:"AbnormalClaims" json:"AbnormalClaims" toml:"AbnormalClaims" yaml:"AbnormalClaims"`
}
...
func (r *outputR) GetTransaction() *Transaction {
	if r == nil {
		return nil
	}
...
func (r *outputR) GetAbnormalClaims() AbnormalClaimSlice {
	if r == nil {
		return nil
	}
	return r.AbnormalClaims
}
...
// CountG returns the count of all Output records in the query using the global executor
...
func (o *Output) AbnormalClaims(mods ...qm.QueryMod) abnormalClaimQuery {
	...
	return AbnormalClaims(queryMods...)
}
...
func (outputL) LoadTransaction(e boil.Executor, singular bool, maybeOutput interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeOutput.(*Output)
		if !ok {
			object = new(Output)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeOutput)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeOutput))
			}
		}
	} else {
		s, ok := maybeOutput.(*[]*Output)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeOutput)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeOutput))
			}
		}
	}
	...
	query := NewQuery(
		qm.From(`transaction`),
		qm.WhereIn(`transaction.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (outputL) LoadAbnormalClaims(e boil.Executor, singular bool, maybeOutput interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`abnormal_claim`),
		qm.WhereIn(`abnormal_claim.output_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (q outputQuery) UpdateAllGP(cols M) {
...
func (q outputQuery) DeleteAllG() error {
...
func (q outputQuery) DeleteAllGP() {
...
func (o *Output) Exists(exec boil.Executor) (bool, error) {
	return OutputExists(exec, o.ID)
}

package migration

import (
	"bytes"
	"compress/gzip"
	"fmt"
	"io"
	"strings"
)

func bindata_read(data []byte, name string) ([]byte, error) {
	gz, err := gzip.NewReader(bytes.NewBuffer(data))
	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	var buf bytes.Buffer
	_, err = io.Copy(&buf, gz)
	gz.Close()

	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	return buf.Bytes(), nil
}

var _migration_000_init_schema_sql = []byte("")

func migration_000_init_schema_sql() ([]byte, error) {
	return bindata_read(
		_migration_000_init_schema_sql,
		"migration/000_init_schema.sql",
	)
}

var _migration_001_supports_sql = []byte("")

func migration_001_supports_sql() ([]byte, error) {
	return bindata_read(
		_migration_001_supports_sql,
		"migration/001_supports.sql",
	)
}

var _migration_002_decimals_sql = []byte("")

func migration_002_decimals_sql() ([]byte, error) {
	return bindata_read(
		_migration_002_decimals_sql,
		"migration/002_decimals.sql",
	)
}

var _migration_003_dht_tracking_sql = []byte("")

func migration_003_dht_tracking_sql() ([]byte, error) {
	return bindata_read(
		_migration_003_dht_tracking_sql,
		"migration/003_dht_tracking.sql",
	)
}

var _migration_004_new_indices_sql = []byte("")

func migration_004_new_indices_sql() ([]byte, error) {
	return bindata_read(
		_migration_004_new_indices_sql,
		"migration/004_new_indices.sql",
	)
}

var _migration_005_remove_foreign_keys_sql = []byte("")

func migration_005_remove_foreign_keys_sql() ([]byte, error) {
	return bindata_read(
		_migration_005_remove_foreign_keys_sql,
		"migration/005_remove_foreign_keys.sql",
	)
}

var _migration_006_add_height_index_sql = []byte("")

func migration_006_add_height_index_sql() ([]byte, error) {
	return bindata_read(
		_migration_006_add_height_index_sql,
		"migration/006_add_height_index.sql",
	)
}

var _migration_007_more_decimals_sql = []byte("")

func migration_007_more_decimals_sql() ([]byte, error) {
	return bindata_read(
		_migration_007_more_decimals_sql,
		"migration/007_more_decimals.sql",
	)
}

var _migration_008_schema_fix_sql = []byte("")

func migration_008_schema_fix_sql() ([]byte, error) {
	return bindata_read(
		_migration_008_schema_fix_sql,
		"migration/008_schema_fix.sql",
	)
}

var _migration_009_certificate_validation_sql = []byte("")

func migration_009_certificate_validation_sql() ([]byte, error) {
	return bindata_read(
		_migration_009_certificate_validation_sql,
		"migration/009_certificate_validation.sql",
	)
}

var _migration_010_triggers_sql = []byte("")

func migration_010_triggers_sql() ([]byte, error) {
	return bindata_read(
		_migration_010_triggers_sql,
		"migration/010_triggers.sql",
	)
}

var _migration_011_add_license_sql = []byte("")

func migration_011_add_license_sql() ([]byte, error) {
	return bindata_read(
		_migration_011_add_license_sql,
		"migration/011_add_license.sql",
	)
}

var _migration_012_store_last_height_sql = []byte("")

func migration_012_store_last_height_sql() ([]byte, error) {
	return bindata_read(
		_migration_012_store_last_height_sql,
		"migration/012_store_last_height.sql",
	)
}

var _migration_013_add_input_vin_sql = []byte("")

func migration_013_add_input_vin_sql() ([]byte, error) {
	return bindata_read(
		_migration_013_add_input_vin_sql,
		"migration/013_add_input_vin.sql",
	)
}

var _migration_014_yay_new_metadata_sql = []byte("")

func migration_014_yay_new_metadata_sql() ([]byte, error) {
	return bindata_read(
		_migration_014_yay_new_metadata_sql,
		"migration/014_yay_new_metadata.sql",
	)
}

// Asset loads and returns the asset for the given name.
// It returns an error if the asset could not be found or
// could not be loaded.
func Asset(name string) ([]byte, error) {
	cannonicalName := strings.Replace(name, "\\", "/", -1)
	if f, ok := _bindata[cannonicalName]; ok {
		return f()
	}
	return nil, fmt.Errorf("Asset %s not found", name)
}

// AssetNames returns the names of the assets.
func AssetNames() []string {
	names := make([]string, 0, len(_bindata))
	for name := range _bindata {
		names = append(names, name)
	}
	return names
}

// _bindata is a table, holding each asset generator, mapped to its name.
var _bindata = map[string]func() ([]byte, error){
	"migration/000_init_schema.sql":            migration_000_init_schema_sql,
	"migration/001_supports.sql":               migration_001_supports_sql,
	"migration/002_decimals.sql":               migration_002_decimals_sql,
	"migration/003_dht_tracking.sql":           migration_003_dht_tracking_sql,
	"migration/004_new_indices.sql":            migration_004_new_indices_sql,
	"migration/005_remove_foreign_keys.sql":    migration_005_remove_foreign_keys_sql,
	"migration/006_add_height_index.sql":       migration_006_add_height_index_sql,
	"migration/007_more_decimals.sql":          migration_007_more_decimals_sql,
	"migration/008_schema_fix.sql":             migration_008_schema_fix_sql,
	"migration/009_certificate_validation.sql": migration_009_certificate_validation_sql,
	"migration/010_triggers.sql":               migration_010_triggers_sql,
	"migration/011_add_license.sql":            migration_011_add_license_sql,
	"migration/012_store_last_height.sql":      migration_012_store_last_height_sql,
	"migration/013_add_input_vin.sql":          migration_013_add_input_vin_sql,
	"migration/014_yay_new_metadata.sql":       migration_014_yay_new_metadata_sql,
}

// AssetDir returns the file names below a certain
// directory embedded in the file by go-bindata.
// For example if you run go-bindata on data/... and data contains the
// following hierarchy:
//     data/
//       foo.txt
//       img/
//         a.png
//         b.png
// then AssetDir("data") would return []string{"foo.txt", "img"}
// AssetDir("data/img") would return []string{"a.png", "b.png"}
// AssetDir("foo.txt") and AssetDir("notexist") would return an error
// AssetDir("") will return []string{"data"}.
func AssetDir(name string) ([]string, error) {
	node := _bintree
	if len(name) != 0 {
		cannonicalName := strings.Replace(name, "\\", "/", -1)
		pathList := strings.Split(cannonicalName, "/")
		for _, p := range pathList {
			node = node.Children[p]
			if node == nil {
				return nil, fmt.Errorf("Asset %s not found", name)
			}
		}
	}
	if node.Func != nil {
		return nil, fmt.Errorf("Asset %s not found", name)
	}
	rv := make([]string, 0, len(node.Children))
	for name := range node.Children {
		rv = append(rv, name)
	}
	return rv, nil
}

type _bintree_t struct {
	Func     func() ([]byte, error)
	Children map[string]*_bintree_t
}

var _bintree = &_bintree_t{nil, map[string]*_bintree_t{
	"migration": &_bintree_t{nil, map[string]*_bintree_t{
		"000_init_schema.sql":            &_bintree_t{migration_000_init_schema_sql, map[string]*_bintree_t{}},
		"001_supports.sql":               &_bintree_t{migration_001_supports_sql, map[string]*_bintree_t{}},
		"002_decimals.sql":               &_bintree_t{migration_002_decimals_sql, map[string]*_bintree_t{}},
		"003_dht_tracking.sql":           &_bintree_t{migration_003_dht_tracking_sql, map[string]*_bintree_t{}},
		"004_new_indices.sql":            &_bintree_t{migration_004_new_indices_sql, map[string]*_bintree_t{}},
		"005_remove_foreign_keys.sql":    &_bintree_t{migration_005_remove_foreign_keys_sql, map[string]*_bintree_t{}},
		"006_add_height_index.sql":       &_bintree_t{migration_006_add_height_index_sql, map[string]*_bintree_t{}},
		"007_more_decimals.sql":          &_bintree_t{migration_007_more_decimals_sql, map[string]*_bintree_t{}},
		"008_schema_fix.sql":             &_bintree_t{migration_008_schema_fix_sql, map[string]*_bintree_t{}},
		"009_certificate_validation.sql": &_bintree_t{migration_009_certificate_validation_sql, map[string]*_bintree_t{}},
		"010_triggers.sql":               &_bintree_t{migration_010_triggers_sql, map[string]*_bintree_t{}},
		"011_add_license.sql":            &_bintree_t{migration_011_add_license_sql, map[string]*_bintree_t{}},
		"012_store_last_height.sql":      &_bintree_t{migration_012_store_last_height_sql, map[string]*_bintree_t{}},
		"013_add_input_vin.sql":          &_bintree_t{migration_013_add_input_vin_sql, map[string]*_bintree_t{}},
		"014_yay_new_metadata.sql":       &_bintree_t{migration_014_yay_new_metadata_sql, map[string]*_bintree_t{}},
	}},
}}
